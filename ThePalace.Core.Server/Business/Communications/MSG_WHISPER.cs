...
{
    [Description("whis")]
    public struct MSG_WHISPER : IReceiveBusiness
    {
        public void Receive(ThePalaceEntities dbContext, object message)
        {
            ...
            var inboundPacket = (Protocols.MSG_WHISPER)protocol;
            var chatStr = inboundPacket.text;

            Logger.ConsoleLog($"MSG_WHISPER[{sessionState.UserID} -> {inboundPacket.target}]: {chatStr}");
            ...
            {
                sessionState.Send(inboundPacket, EventTypes.MSG_WHISPER, (Int32)sessionState.UserID);

                targetSessionState.Send(inboundPacket, EventTypes.MSG_WHISPER, (Int32)sessionState.UserID);
            }
        }
    }
}
