angular.module('ThePalace').service('magicService', ['$window', 'utilService', function ($window, utilService) {
    ...
    this.gStringCypherTable = (function (seed) {
        ...
        var gSeed = seed ? seed : 1;

        var myRandom = (function (max) {
            var hi = $window.parseInt(gSeed / R_Q);
            var lo = $window.parseInt(gSeed % R_Q);
            var test = $window.parseInt(R_A * lo - R_R * hi);

            if (test > 0)
                gSeed = test;
            else
                gSeed = $window.parseInt(test + R_M);

            return ($window.parseInt((gSeed / R_M) * max) & 0x000000FF);
        });

        for (var idx = 0; idx < 512; idx++) {
            var value = myRandom(256);
    ...
    this.ComputeLicenseCounter = (function (crc, seed) {
        return crc ^ seed ^ gMagicLong;
    });

    this.SeedToWizKey = (function (seedCounter, isPuid) {
        ...
        while (seedCounter) {
            seedStr.push($window.parseInt(charCode_A + ((seedCounter % 13) ^ 4)));
            seedCounter = $window.parseInt(seedCounter / 13);
        }
    ...
    this.WizKeytoSeed = (function (seedStr) {
        var charCode_A = 'A'.charCodeAt(0);
        var charCode_Q = 'Q'.charCodeAt(0);
        var seedCounter = 0;
        var mag = 1;

        for (var idx = 0; idx < seedStr.length; idx++) {
            var code = utilService.ReadByte(seedStr, idx);

            if (code < charCode_A || code > charCode_Q)
                continue;

            seedCounter += ((code - charCode_A) ^ 4) * mag;
            mag *= 13;
        }
    ...
    this.cryptString = (function (source, decrypt) {
        var returnValue = new Array(source.length);
        var lastCharCode = 0;

        for (var i = source.length - 1, rc = 0; i >= 0; --i) {
            var oldCharCode = utilService.ReadByte(source, i);
            var newCharCode = oldCharCode ^ this.gStringCypherTable[rc++] ^ lastCharCode;

            returnValue[i] = utilService.WriteByte(newCharCode);

            lastCharCode = (decrypt ? oldCharCode : newCharCode) ^ this.gStringCypherTable[rc++];
        }

        return utilService.WriteBytes(returnValue);
    });
}]);
